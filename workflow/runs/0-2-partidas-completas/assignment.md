# Asignación: Test de partidas completas 2/4/6 (gate partidas)

- ID: `0-2-partidas-completas` · Rama: `task/0-2-partidas-completas` · Tipo (`--kind`): `default`
- Asignado por: Claude (supervisor) · Worker: Hermes perfil `trucoai` · Aprueba: Emmanuel

## Qué hacer

Implementá **exactamente** la historia BMAD `docs/stories/0-2-partidas-completas.md`: sus criterios de aceptación son el objetivo observable
...
```json
["e2e/*", "playwright.config.ts", "package.json", "package-lock.json", ".github/*", "workflow/config.json", "docs/planning/test-strategy.md", "docs/project-context.md", "docs/stories/0-2-partidas-completas.md"]
```

Además podés escribir en `workflow/runs/0-2-partidas-completas/` (implementation.md). Cualquier otro archivo tocado hace fallar el gate `alcance`.
...
3. Commits chicos en la rama `task/0-2-partidas-completas` con mensajes `feat(engine): …` / `test(engine): …` / `chore: …`.
4. Corré `wf verify -i 0-2-partidas-completas` hasta que esté en verde **sobre el último commit**.
5. Completá la sección **Dev Agent Record** de `docs/stories/0-2-partidas-completas.md` (modelo, notas, lista de archivos) y cambiá su `Status:` a `review`.
6. Escribí `workflow/runs/0-2-partidas-completas/implementation.md`: resumen, archivos, salida de los comandos de verificación, riesgos y cualquier desvío de la historia.
7. Commit final y `wf record-impl -i 0-2-partidas-completas`.

## Criterios de aceptación

- [ ] Todos los AC de `docs/stories/0-2-partidas-completas.md` cumplidos y cubiertos por tests.
- [ ] `wf verify -i 0-2-partidas-completas` en verde sobre el commit registrado.
