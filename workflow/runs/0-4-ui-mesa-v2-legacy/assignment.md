# Asignación: UI mesa v2 sobre el juego actual

- ID: `0-4-ui-mesa-v2-legacy` · Rama: `task/0-4-ui-mesa-v2-legacy` · Tipo (`--kind`): `default`
- Asignado por: Claude (supervisor) · Worker: Hermes perfil `trucoai` · Aprueba: Emmanuel

## Qué hacer

Implementá **exactamente** la historia BMAD `docs/stories/0-4-ui-mesa-v2-legacy.md`: sus criterios de aceptación son el objetivo observable
...
```json
["src/ui/*", "src/styles.css", "src/App.ts", "src/main.ts", "index.html", "public/*", "e2e/*", "docs/design/*", "package.json", "package-lock.json", "docs/stories/0-4-ui-mesa-v2-legacy.md"]
```

Además podés escribir en `workflow/runs/0-4-ui-mesa-v2-legacy/` (implementation.md). Cualquier otro archivo tocado hace fallar el gate `alcance`.
...
3. Commits chicos en la rama `task/0-4-ui-mesa-v2-legacy` con mensajes `feat(engine): …` / `test(engine): …` / `chore: …`.
4. Corré `wf verify -i 0-4-ui-mesa-v2-legacy` hasta que esté en verde **sobre el último commit**.
5. Completá la sección **Dev Agent Record** de `docs/stories/0-4-ui-mesa-v2-legacy.md` (modelo, notas, lista de archivos) y cambiá su `Status:` a `review`.
6. Escribí `workflow/runs/0-4-ui-mesa-v2-legacy/implementation.md`: resumen, archivos, salida de los comandos de verificación, riesgos y cualquier desvío de la historia.
7. Commit final y `wf record-impl -i 0-4-ui-mesa-v2-legacy`.

## Criterios de aceptación

- [ ] Todos los AC de `docs/stories/0-4-ui-mesa-v2-legacy.md` cumplidos y cubiertos por tests.
- [ ] `wf verify -i 0-4-ui-mesa-v2-legacy` en verde sobre el commit registrado.
