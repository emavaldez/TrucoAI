# Asignación: Motor v2: observación pública y simulación

- ID: `1-6-observacion-y-simulacion` · Rama: `task/1-6-observacion-y-simulacion` · Tipo (`--kind`): `default`
- Asignado por: Claude (supervisor) · Worker: Hermes perfil `trucoai` · Aprueba: Emmanuel

## Qué hacer

Implementá **exactamente** la historia BMAD `docs/stories/1-6-observacion-y-simulacion.md`: sus criterios de aceptación son el objetivo observable
...
Además podés escribir en `workflow/runs/1-6-observacion-y-simulacion/` (implementation.md). Cualquier otro archivo tocado hace fallar el gate `alcance`.
...
3. Commits chicos en la rama `task/1-6-observacion-y-simulacion` con mensajes `feat(engine): …` / `test(engine): …` / `chore: …`.
4. Corré `wf verify -i 1-6-observacion-y-simulacion` hasta que esté en verde **sobre el último commit**.
5. Completá la sección **Dev Agent Record** de `docs/stories/1-6-observacion-y-simulacion.md` (modelo, notas, lista de archivos) y cambiá su `Status:` a `review`.
6. Escribí `workflow/runs/1-6-observacion-y-simulacion/implementation.md`: resumen, archivos, salida de los comandos de verificación, riesgos y cualquier desvío de la historia.
7. Commit final y `wf record-impl -i 1-6-observacion-y-simulacion`.

## Criterios de aceptación

- [ ] Todos los AC de `docs/stories/1-6-observacion-y-simulacion.md` cumplidos y cubiertos por tests.
- [ ] `wf verify -i 1-6-observacion-y-simulacion` en verde sobre el commit registrado.
