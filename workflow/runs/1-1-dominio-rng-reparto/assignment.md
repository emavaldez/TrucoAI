# Asignación: Motor v2: dominio, RNG, reparto y rotación

- ID: `1-1-dominio-rng-reparto` · Rama: `task/1-1-dominio-rng-reparto` · Tipo (`--kind`): `default`
- Asignado por: Claude (supervisor) · Worker: Hermes perfil `trucoai` · Aprueba: Emmanuel

## Qué hacer

Implementá **exactamente** la historia BMAD `docs/stories/1-1-dominio-rng-reparto.md`: sus criterios de aceptación son el objetivo observable
...
Además podés escribir en `workflow/runs/1-1-dominio-rng-reparto/` (implementation.md). Cualquier otro archivo tocado hace fallar el gate `alcance`.
...
3. Commits chicos en la rama `task/1-1-dominio-rng-reparto` con mensajes `feat(engine): …` / `test(engine): …` / `chore: …`.
4. Corré `wf verify -i 1-1-dominio-rng-reparto` hasta que esté en verde **sobre el último commit**.
5. Completá la sección **Dev Agent Record** de `docs/stories/1-1-dominio-rng-reparto.md` (modelo, notas, lista de archivos) y cambiá su `Status:` a `review`.
6. Escribí `workflow/runs/1-1-dominio-rng-reparto/implementation.md`: resumen, archivos, salida de los comandos de verificación, riesgos y cualquier desvío de la historia.
7. Commit final y `wf record-impl -i 1-1-dominio-rng-reparto`.

## Criterios de aceptación

- [ ] Todos los AC de `docs/stories/1-1-dominio-rng-reparto.md` cumplidos y cubiertos por tests.
- [ ] `wf verify -i 1-1-dominio-rng-reparto` en verde sobre el commit registrado.
