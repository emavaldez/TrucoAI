...
#   bash training/train.sh r2            # si existe training/config/r2.json, usa esa
...
CONFIG="training/config/$RUN.json"
[ -f "$CONFIG" ] || CONFIG="training/config/default.json"
stop=0
trap 'stop=1' INT TERM
while [ "$stop" -eq 0 ]; do
  caffeinate -dimsu python training/learner/run.py train --run "$RUN" --config "$CONFIG" "$@" 2>&1 \
