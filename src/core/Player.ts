...
export class Player {
  ...
  calculateEnvido(): number {
    ...
    for (const [suit, count] of Object.entries(suitCounts)) {
      if (count >= 2) {
        ...
        suitCards.sort((a, b) => (b as any).number - (a as any).number);
    ...
    if (maxScore === 0 && this.hand.length > 0) {
      maxScore = Math.max(...this.hand.map(c => c.number));
    }
  ...
  getTrucoStrength(): number {
    ...
    const avgValue = this.hand.reduce((sum, c) => sum + (c as any).number, 0) / (this.hand.length || 1);
    strength += avgValue * 2;
